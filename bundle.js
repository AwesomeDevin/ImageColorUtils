class ColorUtils {
    constructor({ leftTopPosition = [0, 0], rightBottomPosition = [1, 1], movePxConfig = 30, boundaryValue = 10 }) {
    ...
    compare(oldVal, newVal, boundaryValue) {
        return ColorUtils.isAdjust(oldVal, newVal, boundaryValue || 10);
    }
    static getAverage(data, valueType) {
        ...
        return valueType === 'rgb' ?
            ...
            :
                ColorUtils.RGB2HSL(Math.round(total[0] / data.length), Math.round(total[1] / data.length), Math.round(total[2] / data.length));
    }
    ...
    leftTopMockMove({ originHslMedia, imageData, width }) {
        ...
        for (let count = 1; count <= movePxConfig; count++) {
            ...
            if (ColorUtils.isAdjust(originHslMedia[key], mockHslMedia, this.boundaryValue)) {
    ...
    rightBottomMockMove({ originHslMedia, imageData, width }) {
        const movePxConfig = this.movePxConfig;
